...
func parseV2IOFn(stats *IOStats) func([]string) error {
	return func(fields []string) error {
		if len(fields) < 2 {
			reportError(&ValueError{wrappedError{Err: fmt.Errorf("malformed line fields: '%v'", fields)}})
		}
		...
		for i := 1; i < len(fields); i++ {
			...
			if len(parts) != 2 {
				reportError(&ValueError{wrappedError{Err: fmt.Errorf("unexpected format in io file,part: %d, content: %v", i, fields[i])}})
				continue
			}
