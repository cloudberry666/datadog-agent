...
func parseFile(fr fileReader, path string, p parser) error {
	f, err := fr.open(path)
	if err != nil {
		return &FileSystemError{wrappedError{Err: err}}
	}
	defer f.Close()
...
func parseSingleSignedStat(fr fileReader, path string, val **int64) error {
	return parseFile(fr, path, func(line string) error {
		...
		if err != nil {
			return &ValueError{wrappedError{Err: err}}
		}
		*val = &value
		return &stopParsingError{}
	})
...
func parse2ColumnStatsWithMapping(fr fileReader, path string, keyColumn, valueColumn int, mapping map[string]**uint64) error {
	return parse2ColumnStats(fr, path, keyColumn, valueColumn, func(key, value string) error {
		if storeVal, found := mapping[key]; found {
			...
			if err != nil {
				reportError(&ValueError{wrappedError{Err: err}})
			}
...
func parsePSI(fr fileReader, path string, somePsi, fullPsi *PSIStats) error {
	return parseColumnStats(fr, path, func(fields []string) error {
		if len(fields) != 5 {
			reportError(&ValueError{wrappedError{Err: fmt.Errorf("unexpected format for psi file at: %s, line content: %v", path, fields)}})
			return nil
		}
		...
		if fields[0] == "some" {
			...
			reportError(&ValueError{wrappedError{Err: fmt.Errorf("unexpected psi type (some|full) for psi file at: %s, type: %s", path, fields[0])}})
		}
		...
		for i := 1; i < 5; i++ {
			...
			if len(parts) != 2 {
				reportError(&ValueError{wrappedError{Err: fmt.Errorf("unexpected format for psi file at: %s, part: %d, content: %v", path, i, fields[i])}})
				continue
			}
